/**
 * Batch Translation Script for MGS Application
    ...
    ejsFiles.forEach(filePath => {
        try {
            ...
            if (originalContent !== translatedContent) {
                ...
                translatedCount++;
                console.log(`âœ… ${path.relative(viewsDir, filePath)}`);
            } else {
                skippedCount++;
            }
        } catch (error) {
            console.error(`âŒ Error: ${path.relative(viewsDir, filePath)} - ${error.message}`);
        }
    });

    console.log('\nðŸ“Š Translation Summary:');
    console.log(`   âœ… Translated: ${translatedCount} files`);
    console.log(`   â­ï¸  Skipped: ${skippedCount} files`);
    console.log(`   ðŸ“ Total: ${ejsFiles.length} files`);
