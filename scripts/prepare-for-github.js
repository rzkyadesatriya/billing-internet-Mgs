...
function removeSensitiveFiles() {
    ...
    console.log('ðŸ—‘ï¸  Menghapus file sensitif...');
    ...
    sensitiveFiles.forEach(file => {
        ...
        if (fs.existsSync(filePath)) {
            try {
                ...
                console.log(`  âœ… Dihapus: ${file}`);
            } catch (error) {
                console.log(`  âš ï¸  Gagal menghapus: ${file} - ${error.message}`);
            }
        } else {
            console.log(`  â„¹ï¸  Tidak ditemukan: ${file}`);
        }
    });
}
...
function createDummyConfigFiles() {
    console.log('\nðŸ“ Membuat file konfigurasi dummy...');
    ...
    try {
        ...
        console.log('  âœ… Dibuat: config/superadmin.txt (kosong)');
    } catch (error) {
        console.log(`  âš ï¸  Gagal membuat superadmin.txt: ${error.message}`);
    }
...
`;
    ...
    try {
        ...
        console.log('  âœ… Dibuat: .env.example');
    } catch (error) {
        console.log(`  âš ï¸  Gagal membuat .env.example: ${error.message}`);
    }
}

// Fungsi untuk membersihkan file konfigurasi
function cleanConfigFiles() {
    console.log('\nðŸ§¼ Membersihkan file konfigurasi...');
    ...
    configFiles.forEach(file => {
        console.log(`  â„¹ï¸  Periksa dan pastikan tidak ada data sensitif di: ${file}`);
    });
}

// Fungsi untuk membuat database kosong
function createEmptyDatabase() {
    console.log('\nðŸ’¾ Membuat database kosong...');
    
    // Membuat file database kosong untuk referensi
    const emptyDbPath = path.join(__dirname, '..', 'data', 'empty-database.sql');
    const emptyDbContent = `-- Database schema kosong untuk MGS
...
`;
    ...
    try {
        ...
        console.log('  âœ… Dibuat: data/empty-database.sql');
    } catch (error) {
        console.log(`  âš ï¸  Gagal membuat empty-database.sql: ${error.message}`);
    }
}

// Fungsi untuk membuat README untuk data
function createDataReadme() {
    console.log('\nðŸ“„ Membuat dokumentasi data...');
    
    const readmePath = path.join(__dirname, '..', 'DATA_README.md');
    const readmeContent = `# Data Management untuk MGS

## Struktur Database

Database menggunakan SQLite dan struktur tabel didefinisikan dalam file migrasi di folder [\`migrations/\`](file:///e:/MGS211025/migrations).
...
Semua file migrasi berada di folder [\`migrations/\`](file:///e:/MGS211025/migrations) dan dijalankan secara berurutan berdasarkan nama file.

## Konfigurasi Environment

Salin file [.env.example](file:///e:/MGS211025/.env.example) ke .env dan sesuaikan nilainya:
...
`;
    ...
    try {
        ...
        console.log('  âœ… Dibuat: DATA_README.md');
    } catch (error) {
        console.log(`  âš ï¸  Gagal membuat DATA_README.md: ${error.message}`);
    }
}

// Fungsi utama
async function main() {
    console.log('ðŸš€ Memulai persiapan project untuk GitHub...\n');
    ...
    try {
        ...
        console.error('\nâŒ Terjadi kesalahan:', error.message);
        process.exit(1);
    }
}
