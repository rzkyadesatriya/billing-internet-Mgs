...
async function getPPPoEUsers(options = {}) {
    ...
    if (mode === 'radius') {
        ...
        // Ambil semua secret PPPoE (fallback ke proplist minimal jika query full tidak mengembalikan data)
        let pppSecrets = [];
        try {
            pppSecrets = await conn.write('/ppp/secret/print');
        } catch (err) {
            logger.warn(`Full PPPoE secret print failed, trying minimal proplist: ${err.message}`);
        }
        if (!Array.isArray(pppSecrets) || pppSecrets.length === 0) {
            try {
                pppSecrets = await conn.write('/ppp/secret/print', ['=.proplist=.id,name,profile,disabled,service']);
            } catch (err) {
                logger.error(`Minimal PPPoE secret print failed: ${err.message}`);
                pppSecrets = [];
            }
        }

        // Ambil koneksi aktif dengan timeout singkat agar list user tidak ikut gagal saat /ppp/active lambat
        let activeNames = [];
        try {
            const activeResult = await Promise.race([
                getActivePPPoEConnections(options),
            ...
            if (activeResult && activeResult.success && Array.isArray(activeResult.data)) {
                activeNames = activeResult.data.map(c => c && c.name).filter(Boolean);
            }
        } catch (err) {
            logger.warn(`Active PPPoE lookup skipped: ${err.message}`);
        }
        // Gabungkan data
...
function getUnknownParamFromMikrotikError(error) {
    const msg = String((error && error.message) || '');
    const m = msg.match(/unknown parameter\s+([a-zA-Z0-9-]+)/i);
    return m ? m[1] : null;
}

async function writeWithUnknownParamFallback(conn, command, params) {
    let currentParams = Array.isArray(params) ? [...params] : [];
    const removedParams = [];

    for (let attempt = 0; attempt < 6; attempt++) {
        try {
            await conn.write(command, currentParams);
            return { success: true, removedParams };
        } catch (error) {
            const unknownParam = getUnknownParamFromMikrotikError(error);
            if (!unknownParam) throw error;

            const before = currentParams.length;
            currentParams = currentParams.filter(p => !String(p).startsWith(`=${unknownParam}=`));

            if (currentParams.length === before) throw error;

            removedParams.push(unknownParam);
            logger.warn(`[MIKROTIK] ${command}: unknown parameter '${unknownParam}', retry tanpa parameter tersebut`);
        }
    }

    throw new Error('Gagal memproses parameter profile hotspot (terlalu banyak parameter tidak didukung)');
}
...
async function addHotspotProfile(profileData, options = {}) {
    try {
        ...
        await writeWithUnknownParamFallback(conn, '/ip/hotspot/user/profile/add', params);
...
async function editHotspotProfile(profileData, options = {}) {
    try {
        ...
        await writeWithUnknownParamFallback(conn, '/ip/hotspot/user/profile/set', params);
