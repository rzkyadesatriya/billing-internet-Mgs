/**
 * FIXED VERSION - adminMappingNew.js
...
router.get('/api/mapping/new', adminAuth, async (req, res) => {
    try {
        ...
        const [
            ...
            // Load ONU devices from GenieACS
            new Promise(async (resolve, reject) => {
                try {
                    console.log('🔍 Loading ONU devices from GenieACS...');
                    
                    // Simulasi data GenieACS (ganti dengan kode asli Anda)
                    const genieacsDevices = []; // Ini akan diisi dengan data dari GenieACS
                    ...
                    for (const device of genieacsDevices) {
                        try {
                            ...
                            // 2. REMOVED: Serial number lookup karena kolom tidak ada di database
                            // Jika diperlukan, bisa ditambahkan ke tabel customers atau gunakan alternatif
                    ...
                    console.log('🔍 Sample device:', JSON.stringify(devicesWithCoords[0], null, 2));
                    ...
                    // FIXED: Pastikan customers sudah tersedia sebelum digunakan
