...
const isAutoCustomerLoginEnabled = () => {
    const v = getSetting('customer_auto_login', false);
...
async function tryAutoLoginCustomer(req) {
    if (!isAutoCustomerLoginEnabled()) return false;
    if (req.session && (req.session.phone || req.session.customer_phone)) return true;
...
const ensureCustomerSession = async (req, res, next) => {
    try {
        if ((!req.session?.customer_username) && !(req.session?.phone || req.session?.customer_phone)) {
