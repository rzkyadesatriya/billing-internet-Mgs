    ...
    try {
        ...
        if (typeof mergedSettings.mikrotik_routers === 'string') {
            try {
                ...
                const normalized = [];
                const ids = new Set();
                const oldRouters = Array.isArray(oldSettings.mikrotik_routers) ? oldSettings.mikrotik_routers : [];
                const oldRoutersById = new Map(
                    oldRouters
                        .filter(r => r && typeof r === 'object' && String(r.id ?? '').trim())
                        .map(r => [String(r.id).trim(), r])
                );
                const removedRouterIdsRaw = String(newSettings.mikrotik_removed_router_ids ?? '').trim();
                const removedRouterIds = new Set(
                    removedRouterIdsRaw
                        .split(',')
                        .map(v => String(v).trim())
                        .filter(Boolean)
                ...
                for (const r of routers) {
                    ...
                    ids.add(id);
                    const previous = oldRoutersById.get(id) || {};
                    const name = String(r.name ?? '').trim() || String(previous.name ?? '').trim() || id;
                    const host = String(r.host ?? '').trim() || String(previous.host ?? '').trim();
                    const port = String(r.port ?? '').trim() || String(previous.port ?? '').trim() || '8728';
                    const user = String(r.user ?? '').trim() || String(previous.user ?? '').trim();
                    const password = String(r.password ?? '').trim() || String(previous.password ?? '').trim();
                    normalized.push({
                        id,
                        name,
                        host,
                ...
                // Safety merge: keep old routers if they are missing from payload and not explicitly removed from UI.
                for (const oldRouter of oldRouters) {
                    if (!oldRouter || typeof oldRouter !== 'object') continue;
                    const oldId = String(oldRouter.id ?? '').trim();
                    if (!oldId) continue;
                    if (ids.has(oldId)) continue;
                    if (removedRouterIds.has(oldId)) continue;

                    ids.add(oldId);
                    normalized.push({
                        id: oldId,
                        name: String(oldRouter.name ?? oldId).trim() || oldId,
                        host: String(oldRouter.host ?? '').trim(),
                        port: String(oldRouter.port ?? '8728').trim() || '8728',
                        user: String(oldRouter.user ?? '').trim(),
                        password: String(oldRouter.password ?? '').trim(),
                        enabled: oldRouter.enabled !== false
                    });
                }

                mergedSettings.mikrotik_routers = normalized;
                delete mergedSettings.mikrotik_removed_router_ids;
...
router.post('/telegram/test', async (req, res) => {
    try {
        ...
        await telegramBot.bot.sendMessage(chatId, '✅ Ini adalah pesan test dari sistem MGS-Billing.');
...
# Generated by MGS-Billing System
# Date: ${timestamp}
...
/ip firewall address-list add list=blocked_customers address=0.0.0.0 comment="Placeholder - Auto managed by MGS-Billing"
...
/ip firewall filter add chain=forward src-address-list=blocked_customers action=drop comment="Block suspended customers (static IP) - MGS-Billing" place-before=0

# Rule 2: Block access to router dari blocked customers (INPUT chain)
/ip firewall filter add chain=input src-address-list=blocked_customers action=drop comment="Block suspended customers from accessing router (static IP) - MGS-Billing"
