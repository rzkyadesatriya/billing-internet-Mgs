...
router.post('/sell-voucher', requireAgentAuth, async (req, res) => {
    try {
        ...
        const shouldSendWa = sendNotification !== false && !!customerPhone;
        logger.info(`Sell voucher request: agent=${agentId}, package=${packageId}, customerPhone=${customerPhone || '-'}, sendWA=${shouldSendWa}`);
        ...
        if (result.success) {
            ...
            // Send WhatsApp notification (best-effort, but return status to UI)
            let waStatus = {
                enabled: shouldSendWa,
                success: false,
                message: shouldSendWa ? 'Belum dikirim' : 'Pengiriman WhatsApp tidak diaktifkan atau nomor kosong'
            };
            try {
                ...
                const voucherData = {
                    ...
                    commission: result.commissionAmount || result.commission || packageData.commissionAmount || 0
                };

                if (shouldSendWa) {
                    let isWaConnected = true;
                    try {
                        const whatsapp = require('../config/whatsapp');
                        const waState = whatsapp?.getWhatsAppStatus ? whatsapp.getWhatsAppStatus() : null;
                        isWaConnected = !!(waState && waState.connected);
                        if (!isWaConnected) {
                            waStatus = {
                                enabled: true,
                                success: false,
                                message: `WhatsApp belum terhubung (${waState?.status || 'disconnected'})`
                            };
                        }
                    } catch (waStateErr) {
                        logger.warn('Unable to read WhatsApp status in sell-voucher route:', waStateErr);
                    }
                    ...
                    waStatus = {
                        ...
                        message: waResult?.message || 'Status pengiriman WhatsApp tidak diketahui'
                    };
                    if (!waStatus.success) {
                        logger.warn(`WA voucher not sent to customer ${customerPhone}: ${waStatus.message}`);
                    }
                    }
                } else if (agent?.phone) {
                    // Keep agent receipt if customer WA is not requested
                ...
                waStatus = {
                    enabled: shouldSendWa,
                    success: false,
                    message: whatsappError?.message || 'Gagal mengirim notifikasi WhatsApp'
                };
            }
            
            res.json({
                success: true,
                message: waStatus.enabled && !waStatus.success
                    ? `Voucher berhasil dijual, tetapi WA gagal dikirim: ${waStatus.message}`
                    : 'Voucher berhasil dijual',
                voucherCode: result.voucherCode,
