...
async function withTimeout(taskPromise, timeoutMs = 10000, message = 'Request timeout') {
  let timer = null;
  try {
...
async function tryAutoLoginCustomer(req) {
  ...
  if (req.session && req.session.phone) return true;

  const configuredPhone = String(getSetting('customer_auto_login_phone', '') || '').trim();
  let customer = null;

  if (configuredPhone) {
    customer = await billingManager.getCustomerByPhone(configuredPhone);
  }

  if (!customer) {
    const customers = await billingManager.getCustomers();
    if (Array.isArray(customers) && customers.length > 0) {
      customer = customers.find(c => c && c.phone) || customers[0];
    }
  }
  ...
  req.session.customer_username = customer.username || `temp_${sessionPhone}`;
...
function addAdminNumber(customerData) {
  const adminNumber = getSetting('admins.0', '6285778015569');
  const companyHeader = getSetting('company_header', 'MAYA GLOBAL SOLUTION');
...
router.get('/login', (req, res) => {
  const settings = getSettingsWithCache();
  tryAutoLoginCustomer(req)
    .then((loggedIn) => {
      if (loggedIn) return res.redirect('/customer/dashboard');
      return res.render('login', { settings, error: null });
    })
    .catch(() => res.render('login', { settings, error: null }));
});

// GET: Base customer portal - redirect appropriately
router.get('/', async (req, res) => {
  try {
    await tryAutoLoginCustomer(req);
  } catch (_) { }
