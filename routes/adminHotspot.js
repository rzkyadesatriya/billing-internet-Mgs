...
async function getAllHotspotUsersCached(routerOptions = {}, timeoutMs = 4000, ttlMs = 60000) {
    const cacheKey = getRouterCacheKey(routerOptions.routerId);
    const cached = hotspotAllUsersCache.get(cacheKey);
    const now = Date.now();
    ...
    try {
        const users = await withTimeout(
            fetchAllHotspotUsers(routerOptions),
            timeoutMs,
            'Timeout saat mengambil semua user hotspot dari Mikrotik'
        );
        hotspotAllUsersCache.set(cacheKey, { ts: now, data: users });
        return users;
    } catch (error) {
        if (cached && Array.isArray(cached.data)) {
            return cached.data;
...
router.get('/', async (req, res) => {
    try {
        ...
        const [activeUsersResult, profilesResult, allUsersResult, voucherOnlineSettingsResult] = await Promise.all([
            withTimeout(
                getActiveHotspotUsers(routerOptions),
                4000,
                'Timeout saat mengambil data user hotspot aktif dari Mikrotik'
            ).catch((error) => ({ success: false, data: [], message: error.message })),
            withTimeout(
                getHotspotProfiles(routerOptions),
                4000,
                'Timeout saat mengambil profile hotspot dari Mikrotik'
            ).catch((error) => ({ success: false, data: [], message: error.message })),
            getAllHotspotUsersCached(routerOptions, 4000, 60000),
            withTimeout(
                getVoucherOnlineSettings(routerOptions),
                3000,
                'Timeout saat mengambil setting voucher online'
            ).catch(() => ({}))
        ]);

        const users = (activeUsersResult && activeUsersResult.success && Array.isArray(activeUsersResult.data)) ? activeUsersResult.data : [];
        const profiles = (profilesResult && profilesResult.success && Array.isArray(profilesResult.data)) ? profilesResult.data : [];
        ...
        const voucherOnlineSettings = (voucherOnlineSettingsResult && typeof voucherOnlineSettingsResult === 'object') ? voucherOnlineSettingsResult : {};
...
router.post('/delete', async (req, res) => {
    ...
    try {
        ...
        invalidateHotspotUsersCache(routerId);
...
router.get('/voucher', async (req, res) => {
    try {
        ...
        const [profilesResult, serversResult, allUsersResult, activeUsersResult, voucherOnlineSettingsResult] = await Promise.all([
            withTimeout(
                getHotspotProfiles(routerOptions),
                4000,
                'Timeout saat mengambil profile hotspot dari Mikrotik'
            ...
            withTimeout(
                getHotspotServers(routerOptions),
                4000,
                'Timeout saat mengambil server hotspot dari Mikrotik'
            ).catch((error) => ({ success: false, data: [], message: error.message })),
            getAllHotspotUsersCached(routerOptions, 4500, 45000),
            withTimeout(
                getActiveHotspotUsers(routerOptions),
                4000,
                'Timeout saat mengambil user hotspot aktif dari Mikrotik'
        ...
        const profiles = (profilesResult && profilesResult.success && Array.isArray(profilesResult.data)) ? profilesResult.data : [];
        const servers = (serversResult && serversResult.success && Array.isArray(serversResult.data)) ? serversResult.data : [];
        const allUsers = Array.isArray(allUsersResult) ? allUsersResult : [];
        const activeUsernames = (activeUsersResult && activeUsersResult.success && Array.isArray(activeUsersResult.data))
            ? activeUsersResult.data.map(user => user.user || user.name).filter(Boolean)
            : [];
        ...
        const voucherOnlineSettings = (voucherOnlineSettingsResult && typeof voucherOnlineSettingsResult === 'object')
            ? voucherOnlineSettingsResult
            : {};
