    ...
    start() {
        ...
        const sync = () => {
            try {
                ...
                console.log('ðŸ“± Technician numbers synced from settings.json');
            } catch (e) {
        ...
        sync(); // Initial sync
        console.log('ðŸ”„ Technician auto-sync enabled - settings.json changes will auto-update technicians');
    }
};
...
app.get('/isolir', async (req, res) => {
    try {
        ...
        const settings = getSettingsWithCache();
        const companyHeader = getSetting('company_header', 'MGS');
...
try {
    whatsapp.connectToWhatsApp().then(sock => {
        if (sock) {
            ...
            console.log('ðŸ¤– Agent WhatsApp Commands initialized');
...
function startServer(portToUse) {
    ...
    try {
        const server = app.listen(port, () => {
            ...
            logger.info(`ðŸŒ Web Portal tersedia di: http://localhost:${port}`);
            ...
            if (err.code === 'EADDRINUSE') {
                logger.error(`âŒ ERROR: Port ${port} sudah digunakan oleh aplikasi lain!`);
                logger.error(`ðŸ’¡ Solusi: Hentikan aplikasi yang menggunakan port ${port} atau ubah port di settings.json`);
                logger.error(`ðŸ” Cek aplikasi yang menggunakan port: netstat -ano | findstr :${port}`);
            } else {
                logger.error('âŒ Error starting server:', err.message);
            }
            process.exit(1);
        });
    } catch (error) {
        logger.error(`âŒ Terjadi kesalahan saat memulai server:`, error.message);
        process.exit(1);
    }
}
...
//             if (result.data) {
//                 logger.info(`ðŸ“‹ IP Server: ${result.data.serverIP}`);
//                 logger.info(`ðŸ“‹ GenieACS URL: ${result.data.genieacsUrl}`);
//                 logger.info(`ðŸ“‹ Script Mikrotik: ${result.data.mikrotikScript}`);
//             }
//         } else {
//             logger.warn(`âš ï¸  Auto GenieACS DNS setup: ${result.message}`);
//         }
//     } catch (error) {
//         logger.error('âŒ Error dalam auto GenieACS DNS setup:', error);
//     }
